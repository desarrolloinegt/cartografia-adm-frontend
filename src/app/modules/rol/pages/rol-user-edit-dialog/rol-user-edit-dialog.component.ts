...
@Component({
  selector: 'app-rol-user-edit-dialog',
  templateUrl: './rol-user-edit-dialog.component.html',
  styleUrls: ['./rol-user-edit-dialog.component.scss']
})
export class RolUserEditDialogComponent {
  ...
  constructor(private userService: UserService, private rolService: RolService, public dialogRef: MatDialogRef<IGroupUserAssignment>, @Inject(MAT_DIALOG_DATA) public data: IGroupUserAssignment, private formBuilder: FormBuilder) {
  ...
  getUsers() {
    this.rolService.getGroupsUsers(this.data.rol_id).subscribe(data => {
  ...
  eliminarUsuario(id:string,nombres: string,apellidos:string) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.rolService.deleteUserToGroup(this.userData).subscribe(resp => {
          if (resp.status == true) {
